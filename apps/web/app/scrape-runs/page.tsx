...
export default async function ScrapeRunsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <form action={createScrapeRun} className="grid gap-3 rounded-[24px] border border-slate-200 bg-white p-6 md:grid-cols-4">
        <input
          name="source"
          placeholder="Source"
        ...
        {data?.items.map((run) => (
          <article key={run.id} className="rounded-[24px] border border-slate-200 bg-white p-6">
            ...
            <div className="mt-4 grid gap-3 text-sm text-slate-600 md:grid-cols-3 xl:grid-cols-6">
              <div className="rounded-2xl bg-slate-50 p-3">Tasks: {run.completed_tasks}/{run.total_tasks}</div>
              ...
              <div className="rounded-2xl bg-slate-50 p-3">Duplicates: {run.total_duplicates}</div>
